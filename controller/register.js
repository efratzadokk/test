...
const KnowmeUser = require('../models/KnowmeUser');
...
const insertIfExsist = async(userEmail) => {
    return new Promise(async(resolve, reject) => {
        ...
        await KnowmeUser.find({ email: userEmail }, async(err, users) => {
            if (users.length) {
                resolve({ message: 'knowmeUser exists' });
                numSessions++;
                return;
            }
            const user = new KnowmeUser({
            ...
            if (success) {  
                ...
                resolve({ message: 'knowmeUser added successfully' });
            }
        })
    })
}
...
const checkPermission = async(req, res) => {
    ...
    try{
    console.log("decodes details " + uid + " " + email)
...
const usernameExistCheck = async (uid) => {
    ...
    return new Promise(async(s, j) => {
      await  KnowmeUser.find({ uid: uid }, (err, $) => {
            console.log($)
            if($.length)
           {
               if($[0].username=="")
...
const usernameCheck = async(req, res) => {
    ...
    KnowmeUser.find({ username: usernameToCheck }, async(err, users) => {
        if (users.length) {
        ...
        KnowmeUser.findOneAndUpdate({'uid' : uid}, {'username' : usernameToCheck}, {upsert: true}, function(err, doc) {
            if (err) {
                console.log("errorrrrr = " + err);
...
const sendWelcomeEmail = async(uid) => {
    ...
    // console.log("jwt", jwt);
    const Cuser = await KnowmeUser.findOne({ uid: uid });
