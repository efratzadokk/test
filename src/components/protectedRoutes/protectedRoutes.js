...
function redirectToLogin(routes) {
    window.location.href = routes ?
        `https://accounts.codes/vlogger/login?routes=${routes}` :
        `https://accounts.codes/vlogger/login`;
    return null
}
...
const ProtectedRoute = ({ component: Component, user, ...rest }) => {
    ...
    useEffect(() => {
        ...
        const url=`https://funnel.dev.leader.codes/api/${userName}/isPermission?isLocal=${isLocal}`;
